...
passerCommande.addEventListener('click', function (e) {
  ...
  // 2ème condition : le panier contient des articles 
    ...
    // Pour tous les champs vides du formulaire : afficher dans la page le message d'erreur (#inputeNameErrorMsg)
    for (let input of allFormInputs) {
      //console.log(input.name);
      let inputName = input.name;
      let inputToValidate = document.querySelector(`#${inputName}`);
      let errorMessage = document.querySelector(`#${inputName}ErrorMsg`);
      //console.log(inputToValidate);
      //console.log(errorMessage);
      if (inputToValidate.value.trim() == "") { // la fonction trim retire les espaces initiaux et finaux
        errorMessage.innerHTML = "Ce champ est requis";
      }
    }
    // Si l'un des champs comporte un message d'erreur du à la non validation avec la fonction test de la Regex spécifique : 
    ...
    // Si l'ensemble des entrées est valide, l'envoi du formulaire est possible
...
/* avant envoi formulaire : 

1/ vérifier if firstName != ""
  si */